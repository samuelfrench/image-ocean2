...
- [x] 2026-05-02 — Gallery auto-scrolling: ambient page-scroll toggle (header button + spacebar, ~30 px/s, wraps to top), live feed (`/api/list?since=` polled every 5 s, prepends new cards with highlight pulse, "+N new ↑" pill when user is scrolled down, smooth scroll-to-top when near top), lightbox slideshow (4 s interval, button + spacebar to toggle, `←`/`→` for manual nav, hover-to-pause).
